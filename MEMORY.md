...
- Public Mock Demo deployment `dpl_EazzAnxDTq3FUwRMGaC8DHggN5E5` is Ready on `https://realtags.davidwang.space`. Demo login and all eight event details verified eight Beijing venues, zero legacy Shanghai venue names, Beijing addresses, and preserved Fixture labeling. The real-user project and DNS were not deployment targets.

## 2026-08-23: Cancelled database cutover recovery

- An Orca-managed Codex session remained `working` in the protected source checkout after cancellation and continued the `realtags-production-db-v2` cutover. It recreated the candidate resource, maintenance variables, and deployments; the database did not independently enter maintenance.
- A safe cancellation must query all Orca worktrees, close only the exact matching agent terminal, and remove the exact clean cutover child worktree before changing Vercel state. Stopping orphaned OS processes or observing Vercel alone is insufficient. Do not terminate the Orca daemon, unrelated agents, or browser processes.
- The delayed operation reached the primary-variable swap under maintenance: the candidate was connected and the source resource was disconnected. Recovery restored both aliases to verified Ready deployment `dpl_9THtmEWsaDHgB6zZkuFmXnkeXnxq`, removed maintenance and prefixed variables, disconnected the candidate, reconnected `realtags-real-user-db` to restore its encrypted variables, and then deleted only the candidate.
...
- A prior ten-check observation window was not sufficient while another Orca agent remained active. Final cancellation evidence requires zero matching active agents, the cutover worktree absent, the original resource connected, the candidate absent, no maintenance or prefixed variables, the expected deployment alias, and a database-backed HTTP probe.
